...
//          ChipN[n] := Point(X, Y);                                         //
          ...
          begin                                                            //
            ...
            if Chip[Y, X].ChipParams[i-3].Value = 0 then Chip[Y, X].ChipParams[i-3].Value := NotSpec; // Если 0, то параметр не измерялся (Арнаутов)
          end;                                                             //
                                                                           //
          Inc(X);                                                          //
...
//        if Assigned(OnEvent) then OnEvent(evError, 'Пустая пластина!');    //
        Close;                                                             //
        ADOQuery.Free();                                                   //
        FormatSettings.DecimalSeparator := '.';                            //
      end;                                                                 //
...
//      end;
      ...
      begin
        ...
        if Diameter = 0 then // Если микросхема
        begin
          X := Ceil(sqrt(NTotal)); // Сделаем
...
////////////////////////////////////////////////////////////////////////////////
    ...
    begin
      ...
      begin
        ...
        Inc(X);         ;
        if X = Length(Chip[0]) then   ////////////// Не задана  SetLength(Chip, Y, X)
        begin
          X := 0;
          Inc(Y);
        end;
...
//    if Trim(Copy(Str, 1, P-1)) = 'FAIL' then
    ...
    P := Pos(#9, Str);
    if Trim(Copy(Str, 1, P-1)) = 'FAIL' then Chip[Y, X].Status := 10 // Пусть будет по НК
                                        else Chip[Y, X].Status := 1;
...
begin                                                                             //
  ...
  begin                                                                           //
    ...
    begin                                                                         //
      ...
      begin                                                                       //
        for i := 1 to P do                                                        //
          if Str[i] = #9 then Inc(NTab); // Найдём кол-во Tabов до координат      //
                                                                                  //
        ChipN[NCh] := GetCoordFromStr(Str, NTab);                                 //
                                                                                  //
        MinX := ChipN[NCh].X; // минимальный X                                    //
        MaxX := ChipN[NCh].X; // максимальный X                                   //
        MaxY := ChipN[NCh].Y; // максимальный Y                                   //
                                                                                  //
        Inc(NCh);                                                                 //
                                                                                  //
        Read_Coord := True;                                                       //
  ...
  begin                                                                           //
    ChipN[NCh].X := ChipN[NCh].X-MinX; // Нормализуем X (уберём отрицательные)    //
                                                                                  //
    Y := Length(Chip)-ChipN[NCh].Y-1; // Отразим по Y (система координат другая)  //
    X := ChipN[NCh].X;                                                            //
                                                                                  //
    Chip[Y, X].ID := NCh+1;                                                       //
    Chip[Y, X].Status := 0;                                                       //
...
begin                                                                                                //
  ...
  try                                                                                                //
    ...
    begin                                                                                            //
      ...
      if S = '' then                                    // Считываем                                 //
      begin                                             // название                                  //
        ErrMess(Handle, 'Не найден столбец статуса !'); // столбцов                                  //
        SL.Free;                                        //                                           //
        Exit;                                           //                                           //
      end;                                              //                                           //
    end;                                                //                                           //
...
begin                                                                        //
  ...
  begin                                                                      //
    if Assigned(OnEvent) then OnEvent(evError, 'Не найден файл Groups.ini'); //
//    Exit;                                                                  //
  end;                                                                       //
                                                                             //
...
////////////////////////////////////
  ...
  SetLength(HistParams, Length(Wafer[0].TestsParams)); // Зададим кол-во групп для гистограмм
...
////////////////////////////////////////////////////////////////////////////////                       //
                                                                                                       //
  try                                                                                                  //
    Workbook1.Sheets['Нормы и режимы'].Activate;                                                       //
  except                                                                                               //
    if Assigned(OnEvent) then OnEvent(evError, 'В шаблоне отсутствует лист <Нормы и режимы>');         //
...
////////////////////////////////////////////////////////////////////////////////
      ...
      BottomSheet := NTotal+17+27; // Самая нижняя строка
          ...
          begin
            if not ToFirstFail then // Если не до 1-го брака
              ...
              for i := 0 to Length(TestsParams)-1 do
                ...
                begin
                  VarMass1[Nm+1, i+1] := '';
                  Chip[Y, X].ChipParams[i].Stat := 4; // Параметр не измерялся
                end;
          end;
...
/////////////////////////////
            ...
            with HistParams[i] do // Для гистограммы
            begin
              ...
              begin                                       //
                Group[m].Num := 0;                        //
                Group[m].Min := MinVal+GroupDiap*m;       // Создадим
                if m < Length(Group)-1 then               // группы
                  Group[m].Max := Group[m].Min+GroupDiap  // для
                else                                      // гистограмм
                  Group[m].Max := MaxVal;                 //
              end;                                        //
              ...
              begin                                              // Распределим
                if ValMass[m] > Group[Nm].Max then               // измерения
                  while (ValMass[m] > Group[Nm].Max) do Inc(Nm); // по
                                                                 // группам
                Inc(Group[Nm].Num);                              //
              end;                                               //
            end;
      ...
      /////////////// ** Нарисуем гистограммы по параметрам ** /////////////////

      SetLength(Chart, 0);
      SetLength(Chart, Length(HistParams));
      Chart[0] := Workbook1.ActiveSheet.ChartObjects(2);             // Первая
      Chart[0].Top := WorkBook1.ActiveSheet.Rows[BottomSheet+3].Top; // гистограмма
      Chart[0].Copy;
      ...
      for i := 0 to Length(HistParams)-1 do
        ...
        begin
          ...
          WorkBook1.ActiveSheet.Cells[BottomSheet+2+26*i, 5].Font.Size := 12;   // Имя
          WorkBook1.ActiveSheet.Cells[BottomSheet+2+26*i, 5] := Name;           // гистограммы
          ...
          Range2 := WorkBook1.ActiveSheet.Range[Cell1, Cell2];                        // данные
      ...
      VarClear(VarMass1); // Очистим
      VarClear(VarMass2); // вариантные массивы
    end; // for Wafer[n]
...
////////////////////////////////////////////////////////////////////////////////
  ...
  for n := 0 to Length(Wafer)-1 do
    ...
    begin
      ...
      BottomSheet := 27+9*(Length(Wafer)-1)+26; // Самая нижняя строка //////// +27
  ...
  begin
    ...
    MinLength[i] := 0; // Если брак
    MaxLength[i] := 0; // то не включать
  end;
...
//////////////////// * Общие гистограммы по параметрам * ///////////////////////
  ...
  for n := 0 to Length(Wafer)-1 do                                                         //
    for i := 0 to Length(HistParams)-1 do                                                  //
      ...
      begin                                                                                // параметры
        ...
        if HistParams[i].AllMaxVal < MaxVal then HistParams[i].AllMaxVal := MaxVal;        // и макс
      end;
  ...
  for i := 0 to Length(HistParams)-1 do
    with HistParams[i] do
    begin
      SortMassByValue(AllValMass); // Отсортируем массивы

      GroupDiap := (AllMaxVal-AllMinVal)/NGroups; // Диапазон группы

      SetLength(Group, NGroups);
      for m := 0 to Length(Group)-1 do            //
      begin                                       //
        Group[m].Num := 0;                        //
        Group[m].Min := AllMinVal+GroupDiap*m;    // Создадим
        if m < Length(Group)-1 then               // группы
          Group[m].Max := Group[m].Min+GroupDiap  // для
        else                                      // гистограмм
      ...
      begin                                                 // Распределим
        if AllValMass[m] > Group[Nm].Max then               // измерения
          while (AllValMass[m] > Group[Nm].Max) do Inc(Nm); // по
                                                            // группам
        Inc(Group[Nm].Num);                                 //
      end;                                                  //
    end;                                                    //
...
//  try
//    Workbook1.Sheets['Гистограммы'].Activate;
//  except
//    if Assigned(OnEvent) then OnEvent(evError, 'В шаблоне отсутствует лист <Гистограммы>');
//
//  end;
//
//  for i := 0 to Length(HistParams)-1 do                        //
...
//      if i > 0 then
//      begin
//        Range1.Copy(WorkBook1.ActiveSheet.Cells[2+25*i, 1]); // Скопируем чарт
//
...
//      Cell2 := WorkBook1.ActiveSheet.Cells[2+NGroups-1, 17+i*3]; // Укажем
//      Range2 := WorkBook1.ActiveSheet.Range[Cell1, Cell2];       // данные
//      Chart[i].Chart.SetSourceData(Range2);                      //
//    end;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  begin
    ...
    begin
      ...
      WorkBook1.ActiveSheet.Cells[2, 2] := 'Брак';
      ...
      WorkBook1.ActiveSheet.Cells[3, 2] := 'Параметр выше нормы';
      ...
      WorkBook1.ActiveSheet.Cells[4, 2] := 'Параметр ниже нормы';
      ...
      WorkBook1.ActiveSheet.Cells[5, 2] := 'Параметр не измерялся';
      ...
      WorkBook1.ActiveSheet.Cells[7, 1] := 'Срез пластины: '+Str;

      WorkBook1.ActiveSheet.Columns[StX].Orientation := 90; // Для отображения
      WorkBook1.ActiveSheet.Columns[StX].Font.Bold := True; // номера
      WorkBook1.ActiveSheet.Columns[StX].Font.Size := 12;   // пластины

      LenX := Length(BlankWafer.Chip[0])+1; // Шаг по Х между пластинами
      ...
      for n := 0 to Length(Wafer)-1 do
        ...
        begin
          ...
          Cell1 := WorkBook1.ActiveSheet.Cells[StY, StX];                           // Объединим
          Cell2 := WorkBook1.ActiveSheet.Cells[StY+Length(BlankWafer.Chip)-1, StX]; // ячейки для
          WorkBook1.ActiveSheet.Range[Cell1, Cell2].Merge;                          // отображения
          WorkBook1.ActiveSheet.Cells[StY, StX] := Num; // Номер пластины           // номеров пластин

          //////////////////////////// * Общий брак * //////////////////////////

          WorkBook1.ActiveSheet.Cells[1, StX+1+LenX div 2] := 'Общая карта';
          ...
          begin
            ...
            begin
              ...
              if n = 0 then                                                                             // Названия
                if NM = 0 then WorkBook1.ActiveSheet.Cells[StY-1, StX+7+i*LenX] := TestsParams[i].Name; // тестов
