...
////////////////////////////////////////////////////////////////////////////////
  ...
  begin
    ...
    if Diameter <> 0 then ChipN[NCh].X := ChipN[NCh].X-MinX; // Скорректируем X (уберём отрицательные)
    Y := ChipN[NCh].Y;
    X := ChipN[NCh].X;
...
//    if Trim(Copy(Str, 1, P-1)) = 'FAIL' then
    ...
    P := Pos(#9, Str);
    if Trim(Copy(Str, 1, P-1)) = 'FAIL' then Chip[Y, X].Status := 10 // Пусть будет по неконтакту
                                        else Chip[Y, X].Status := 1;
